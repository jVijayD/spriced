...
//			}
		...
		if(primaryKeyCount==0) {
			Attribute idAttr = new Attribute(ID,Type.FREE_FORM,DataType.AUTO);

			idAttr.setEditable(false);
			idAttr.setShowInForm(false);
			idAttr.setAutoGenerated(true);
			idAttr.setConstraintType(ConstraintType.PRIMARY_KEY);
			idAttr.setNullable(false);
			idAttr.setAutoGenerated(true);
			idAttr.setSystemAttribute(true);
			idAttr.setDisplayName("Id");
			this.attributes.add(0,idAttr);
		}
		...
		if(nameCount==0) {
			...
			//nameAttr.setSystemAttribute(true);
		...
		if(!isValidPresent) {
			...
			isValidAttr.setSystemAttribute(true);
			isValidAttr.setDisplayName("Is Valid");
		...
		if(!updatedByPresent) {
			...
			updatedByAttr.setSystemAttribute(true);
			updatedByAttr.setDisplayName("Updated By");
		...
		if(!commentPresent) {
			...
			commentAttr.setSystemAttribute(true);
			commentAttr.setDisplayName("Comment");
			this.attributes.add(commentAttr);
		}

		if(codeCount==0){
			Attribute codeAttribute = new Attribute(CODE,Type.FREE_FORM,DataType.AUTO,NAME_SIZE);
			if(!this.getAutoNumberCode()) {
				...
				codeAttribute.setEditable(false);
			}
			...
			codeAttribute.setDisplayName("Code");
